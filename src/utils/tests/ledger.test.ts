...
test('HWProvider login', async () => {
  ...
  const transport: TransportReplayer = await openTransportReplayer(store);

  const ledgerApp = new (LedgerApp as any)(transport);

  const provider = new HWProvider(ledgerApp as any);
